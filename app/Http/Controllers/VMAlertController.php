...
/**
 * VMAlertController - VM-specific alerts reports from PostgreSQL
 * 
 * Provides paginated VM alerts data with filtering and export capabilities.
 * All queries run against PostgreSQL only.
 * 
 * Similar to AlertsReportController but with VM-specific logic.
 * 
 * Requirements: 10.1, 10.5
 */
class VMAlertController extends Controller
{
    /**
    ...
    /**
     * Create a new VMAlertController instance
     * 
     ...
     * GET /api/vm-alerts
     * 
     * Get paginated VM alerts with optional filters - 26 columns with sites JOIN.
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                ...
                'status' => 'nullable|string|in:O,C', // Status filter: O = Open, C = Closed
            ...
            Log::error('Failed to fetch VM alerts', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
     ...
     */
    /**
     * Get VM alerts via partition router
     * 
     * Requirements: 10.1, 10.5
     * 
    ...
    protected function getAlertsViaRouter(
        ...
        try {
            ...
            if (!empty($validated['status'])) {
                $filters['vm_status'] = [$validated['status']]; // Single status: O or C
            } else {
                $filters['vm_status'] = ['O', 'C']; // Both statuses (default)
            }
    ...
    /**
    /**
     * Enrich VM alerts with sites data
     * 
    ...
    private function enrichWithSites($alerts)
    {
        ...
        return collect($alerts)->map(function($alert) use ($siteLookup) {
            ...
            // Calculate aging: closedtime - receivedtime (in hours)
    ...
    /**
     * POST /api/vm-alerts/export/csv/token
     * 
    ...
    public function generateExportToken(Request $request): JsonResponse
    {
        try {
            ...
            Cache::put("vm_export_token:{$token}", [
            ...
            Log::error('Failed to generate VM export token', [
                'error' => $e->getMessage()
            ]);
     ...
     * 
     * Export VM alerts for the selected date to CSV with all 27 columns.
     * Downloads complete data with VM-specific filters.
    ...
    public function exportCsv(Request $request)
    {
        ...
        if (session_status() === PHP_SESSION_ACTIVE) {
            session_write_close();
            Log::info('🔓 Session forcefully closed at start of VM CSV export');
        }
        ...
        try {
            ...
            if ($request->has('token')) {
                ...
                $params = Cache::get("vm_export_token:{$token}");
                ...
                Cache::forget("vm_export_token:{$token}");
            ...
            Log::info('VM CSV export started', [
            ...
            // Generate filename in format: "21 Server VM Alerts Report – DD-MM-YYYY.csv"
            $filename = '21 Server VM Alerts Report – ' . $fromDate->format('d-m-Y') . '.csv';
            ...
            Log::error('Failed to export VM alerts CSV', ['error' => $e->getMessage()]);

            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'EXPORT_ERROR',
                    'message' => 'Failed to export VM alerts CSV',
                    'details' => $e->getMessage(),
                ],
            ], 500);
        }
    ...
    protected function exportViaRouterNoFilters($file, Carbon $startDate, Carbon $endDate, int $limit, int $startOffset, int &$processed, int &$rowNumber): void
    {
        try {
            ...
            $chunkSize = 500; // Reduced chunk size for better memory management
            ...
            while ($processed < $limit) {
                ...
                // Write to CSV and flush output immediately
                foreach ($enriched as $report) {
                    ...
                    // Flush output every 100 records for better streaming
                    if ($processed % 100 == 0) {
                        fflush($file);
                        if (ob_get_level()) {
                            ob_flush();
    ...
    /**
     * Export via partition router WITH filters - downloads filtered VM data
     * 
     * @param resource $file File handle
     * @param Carbon $startDate Start date
     * @param Carbon $endDate End date
    ...
    protected function exportViaRouterWithFilters($file, Carbon $startDate, Carbon $endDate, array $validated, int $limit, int $startOffset, int &$processed, int &$rowNumber): void
    {
        try {
            // Build filter array for partition router
            // Start with VM-SPECIFIC base filters
            $filters = [];
            
            // VM-SPECIFIC: Status filter
            if (!empty($validated['status'])) {
                $filters['vm_status'] = [$validated['status']]; // Single status: O or C
            } else {
                $filters['vm_status'] = ['O', 'C']; // Both statuses (default)
            }
            $filters['vm_sendtoclient'] = 'S'; // Always filter by sendtoclient = 'S'
            ...
            while ($processed < $limit) {
                ...
                foreach ($enriched as $report) {
                    ...
                    // Flush output every 100 records
                    if ($processed % 100 == 0) {
                        fflush($file);
                        if (ob_get_level()) {
                            ob_flush();

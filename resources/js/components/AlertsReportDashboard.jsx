...
import { getAlerts, getFilterOptions, exportCsv, checkCsvReport } from '../services/alertsReportService';
import DashboardLayout from './DashboardLayout';
import api from '../services/api';

const AlertsReportDashboard = () => {
    ...
    const checkCsvReportAvailability = useCallback(async () => {
        // Only check for past dates
        if (!filters.from_date) return;
        
        const selectedDate = new Date(filters.from_date);
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        selectedDate.setHours(0, 0, 0, 0);
        
        // Don't check for current or future dates
        ...
        setCheckingCsv(true);
        try {
            const response = await checkCsvReport(filters.from_date);
            if (response.success && response.exists) {
                setCsvReport({
                    url: response.url,
                    date: response.date,
                    filename: `alerts_report_${response.date}.csv`
                });
            } else {
                setCsvReport(null);
            }
        } catch (err) {
            console.error('Failed to check CSV report:', err);
            setCsvReport(null);
        } finally {
    ...
    useEffect(() => { checkCsvReportAvailability(); }, [checkCsvReportAvailability]);

    const handleFilterChange = (key, value) => setFilters(prev => ({ ...prev, [key]: value }));
    const handleSearch = (e) => { e.preventDefault(); fetchAlerts(1); };
    const handleClearFilters = () => {
        setFilters({ panelid: '', dvrip: '', customer: '', panel_type: '', atmid: '', from_date: serverDate || '', per_page: 25 });
    };
                ...
                <div className="bg-white rounded-lg shadow">
                    <div className="px-4 py-2 border-b border-gray-200">
                        <h2 className="text-sm font-medium text-gray-900 flex items-center">
                            <FilterIcon className="w-4 h-4 mr-2" />Filter Reports
                        </h2>
                    </div>
                    <div className="p-3">
                                ...
                                {loading ? (
                                    ...
                                    alerts.map((a, index) => (
                                        <tr key={a.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap max-w-[100px] truncate" title={a.comment}>{a.comment || '-'}</td>
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap">{a.testing_remark || '-'}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
...
export default AlertsReportDashboard;

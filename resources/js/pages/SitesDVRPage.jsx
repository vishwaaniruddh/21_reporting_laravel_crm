...
import { getDVRSites, getDVRFilterOptions, exportDVRCsv } from '../services/dvrSitesService';
...
const SitesDVRPage = () => {
    ...
    const fetchSites = useCallback(async (page = 1) => {
        ...
        try {
            ...
            const response = await getDVRSites(params);
            if (response.success) {
                setSites(response.data.sites);
                setPagination(response.data.pagination);
            } else {
                setError(response.error?.message || 'Failed to fetch DVR sites');
            }
        } catch (err) {
    ...
    const fetchFilterOptions = useCallback(async () => {
        try {
            const response = await getDVRFilterOptions();
    ...
    const handleClearFilters = () => {
        setFilters({ atmid: '', customer: '', bank: '', dvrname: '', dvrip: '', live: '', per_page: 25 });
    };
        ...
        <DashboardLayout>
            <div className="space-y-3">
                ...
                <div className="bg-white rounded-lg shadow p-4">
                    <h1 className="text-xl font-bold text-gray-900">DVR Sites</h1>
                    <p className="text-sm text-gray-600">Digital Video Recorder sites management</p>
                </div>
                ...
                <div className="bg-white rounded-lg shadow">
                    <div className="px-4 py-2 border-b border-gray-200">
                        <h2 className="text-sm font-medium text-gray-900 flex items-center">
                            <FilterIcon className="w-4 h-4 mr-2" />Filter DVR Sites
                        </h2>
                    </div>
                    <div className="p-3">
                ...
                <div className="bg-white rounded-lg shadow">
                    <div className="px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <h2 className="text-sm font-medium text-gray-900 flex items-center">
                                <ListIcon className="w-4 h-4 mr-2" />DVR Sites
                            </h2>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                            <thead className="bg-gray-50">
                                <tr>
                                    {['SN','Customer','Bank','ATM ID','Address','City','State','Zone','DVR Name','DVR IP','HTTP Port','RTSP Port','Live','Installation Date'].map(h => (
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {loading ? (
                                    <tr><td colSpan="14" className="px-4 py-6 text-center">
                                    ...
                                    <tr><td colSpan="14" className="px-4 py-6 text-center text-gray-500">No DVR sites found matching your criteria.</td></tr>
                                ) : (
                                    sites.map((site) => (
                                        <tr key={site.SN} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap max-w-[200px] truncate" title={site.Address}>{site.Address || '-'}</td>
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap">{site.HTTPPort || '-'}</td>
                                            <td className="px-2 py-1.5 whitespace-nowrap">{site.RTSPPort || '-'}</td>

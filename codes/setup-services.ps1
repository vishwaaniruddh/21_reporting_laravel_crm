...
if (-not (Test-Path $PHP_PATH)) {
    ...
    $phpLocations = @(
        ...
        "C:\wamp64\bin\php\php8.4.11\php.exe"
    )
...
if (-not (Test-Path $NSSM_PATH)) {
    ...
    Write-Host ""
    Write-Host "NSSM is usually at:" -ForegroundColor Yellow
...
Write-Host "OK PHP found: $PHP_PATH" -ForegroundColor Green
Write-Host "OK Project found: $PROJECT_PATH" -ForegroundColor Green
Write-Host "OK NSSM found: $NSSM_PATH" -ForegroundColor Green
Write-Host ""
...
function Create-AlertService {
    ...
    if ($status -eq "SERVICE_RUNNING") {
        Write-Host "OK Service created and started: $DisplayName" -ForegroundColor Green
    } else {
        Write-Host "WARNING Service created but not running: $DisplayName (Status: $status)" -ForegroundColor Yellow
...
if (-not $firewallRule) {
    ...
    Write-Host "OK Firewall rule created for port 9000" -ForegroundColor Green
} else {
